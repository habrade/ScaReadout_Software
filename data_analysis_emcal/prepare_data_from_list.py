...
parser.add_argument("--coarse", type=int, default=0, help="use coarse label")
...
def gen_one_sample(data_file, short_dir, long_dir):
    ...
    int_kind = a.int_kind
    coarse = True if a.coarse > 0 else False

    ret = trigger_fit_square.tri_process(tri, os.path.basename(data_file), coarse=coarse)
    if ret is None:
        return -1
    _, refer, precise_ind, _ = ret
    ...
    label = (precise_ind - actual_start) / freq
    ...
    y_short_noise = np.array(y_short_noise).astype(np.float32) / 36261 + 0.0427
